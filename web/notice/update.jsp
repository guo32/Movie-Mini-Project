...
<div class="container-fluid">
    <div class="container">
        ...
        <%
            ...
            pageContext.setAttribute("notice", noticeController.selectById(id));
        %>
        <form id="register-form" name="registerForm" method="post" action="/notice/update_logic.jsp?id=${notice.id}">
            <h3 class="blog-post-title">
                공지사항 수정하기
            </h3>
            <hr>
            <h2 class="blog-post-title" id="input-for-title">
                <input type="text" name="title" id="title" placeholder="공지사항 제목" value="${notice.title}" class="form-control"/>
            </h2>
            <div class="col-12 mb-2" id="input-for-category">
                <select name="category" id="category" class="form-control">
                    <option selected value="">카테고리를 선택해주세요.</option>
                    <option value="1" <c:if test="${notice.category_id == 1}">selected</c:if>>일반</option>
                    <option value="2" <c:if test="${notice.category_id == 2}">selected</c:if>>회원</option>
                    <option value="3" <c:if test="${notice.category_id == 3}">selected</c:if>>영화</option>
                    <option value="4" <c:if test="${notice.category_id == 4}">selected</c:if>>극장</option>
                </select>
            </div>
            ...
            <div class="text-center mt-2">
                ...
                <div class="btn btn-outline-danger" onclick="location.href='/notice/printOne.jsp?id=${notice.id}'">취소</div>
            </div>
        </form>
